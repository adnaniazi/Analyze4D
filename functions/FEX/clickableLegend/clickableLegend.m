...
for i = 1:length(plothan)
    ...
    setTextColor(objhan(i), obj(groupmem==groups(i)));
end




function setTextColor(hObject, obj)
if get(hObject, 'UserData') == 0 % It is off, set to black color
    set(hObject, 'Color', [1 1 1]* 0.2, 'UserData', false);
    set(obj,'HitTest','off','Visible','off','handlevisibility','off');
else
    set(hObject, 'Color', [1 1 1], 'UserData', true);
