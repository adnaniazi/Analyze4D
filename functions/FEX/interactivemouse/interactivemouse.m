...
% axisH   = get(gcf,'CurrentAxes'); %anan
...
for iFig = 1:length(fH)
    ...
    % Work with each axes
    for iAx = 1 %length(axH) % adnan axesNumber:axesNumber
        ...
        if ismember(opt, {'reset', 'restore', 'restore_orig'})
            ...
            if init
                ...
                if ~isempty(axstuff)
                    ...
                    set(get(axH(iAx), 'children'), ...  %adnan
...
% Set aspect ratios
a= 0;
xLimits = get(axH, 'xlim') + (xyz(1)-(pt(1,1)+pt(2,1))/2)
yLimits = get(axH, 'ylim') + (xyz(2)-(pt(1,2)+pt(2,2))/2)
set(axH, ...
