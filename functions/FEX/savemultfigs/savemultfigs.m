...
% Last Modified by GUIDE v2.5 09-May-2012 14:05:00
...
% set(handles.pushbuttonAbout, 'enable', 'inactive');
% set(handles.pushbuttonAbout, 'visible', 'off');
set(handles.figure1, 'Name', 'Save figure');

warning('off','MATLAB:HandleGraphics:ObsoletedProperty:JavaFrame');
jframe=get(handles.figure1,'javaframe');
path = mfilename('fullpath');
path = path(1:end-numel('functions\FEX\savemultfigs\savemultfigs'));
...
% figlist = findall(0,'Type','fig'); % finds all figures but we need only
% one

figlist = varargin{1,1}{1,1};
...
for i=1:length(figlist)
    ...
    visible = {'off','on'};
    h = uicontrol(parentPanel,'Style','checkbox',...
...
msgbox([{[num2str(nfigsave) ' figure(s) saved in ' num2str(length(filetype)) ' different formats'],...
    },errstr]);
