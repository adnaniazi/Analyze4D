classdef Analyze4D < hgsetget
    %Analyze4D  create a Analyze4D
    %
    %   s = Analyze4D(title,imagefile) creates a splashcreen using
    ...
    %   text to your Analyze4D
    %
    %   Examples:
    %   s = Analyze4D( 'Analyze4D', 'example_splash.png', ...
    %                     'ProgressBar', 'on', ...
    %                     'ProgressPosition', 5, ...
    %                     'ProgressRatio', 0.4 )
    ...
    methods
        ...
        function obj = Analyze4D( title, imagename, varargin )
            % Construct a new splash-screen object
            if nargin<2
                error('Analyze4D:BadSyntax', 'Syntax error. You must supply both a window title and imagename.' );
            end
            ...
            if exist(fullname,'file')~=2
                ...
                if isempty( fullname )
                    error('Analyze4D:BadFile', 'Image ''%s'' could not be found.', imagename );
                end
            end
        ...
        function addText( obj, x, y, text, varargin )
            ...
            %
            %   S.addText(X,Y,STR,...) adds some text to the Analyze4D S
    ...
    methods
        ...
        function set.Visible(obj,val)
            if ~ischar( val ) || ~any( strcmpi( {'on','off'}, val ) )
                error( 'Analyze4D:BadValue', 'Property ''ProgressBar'' must be ''on'' or ''off''' );
            end
        ...
        function set.Border(obj,val)
            if ~ischar( val ) || ~any( strcmpi( {'on','off'}, val ) )
                error( 'Analyze4D:BadValue', 'Property ''Border'' must be ''on'' or ''off''' );
            end
            obj.Border = val;
            obj.updateAll();
        end % set.Border
        ...
        function set.ProgressRatio(obj,val)
            if ~isnumeric( val ) || ~isscalar( val ) || val<0 || val > 1
                error( 'Analyze4D:BadValue', 'Property ''ProgressRatio'' must be a scalar between 0 and 1' );
            end
        ...
        function set.ProgressPosition(obj,val)
            if ~isnumeric( val ) || ~isscalar( val ) || val<1 || val > obj.Height %#ok<MCSUP>
                error( 'Analyze4D:BadValue', 'Property ''ProgressPosition'' must be a vertical position inside the window' );
            end
        ...
        function set.Tag(obj,val)
            if ~ischar( val )
                error( 'Analyze4D:BadValue', 'Property ''Tag'' must be a character array' );
            end
            obj.Tag = val;
        end % set.Tag
    ...
    methods ( Access = private )
        ...
        function createInterfaceComponents( obj, title, imageFile )
            ...
            try
                ...
                error('Analyze4D:BadFile', 'Image ''%s'' could not be loaded.', imageFile );
            end
...
if nargin
    ...
    if numel( params ) ~= numel( values )
        error( 'UIExtras:Analyze4D:BadSyntax', 'Optional arguments must be supplied as parameter-value pairs.' );
    end
        ...
        switch upper( params{ii} )
            ...
            case 'FONTWEIGHT'
                switch upper( values{ii} )
                    ...
                    case 'BOLD'
                        fontWeight = java.awt.Font.PLAIN;
                    otherwise
                        error( 'UIExtras:Analyze4D:BadParameterValue', 'Unsupported FontWeight: %s.', values{ii} );
                end
            case 'FONTANGLE'
                switch upper( values{ii} )
                    case 'NORMAL'
                    ...
                    otherwise
                        error( 'UIExtras:Analyze4D:BadParameterValue', 'Unsupported FontAngle: %s.', values{ii} );
                end
            case 'COLOR'
            ...
            case 'SHADOW'
                if ~ischar( values{ii} ) || ~ismember( upper( values{ii} ), {'ON','OFF'} )
                    error( 'UIExtras:Analyze4D:BadParameter', 'Option ''Shadow'' must be ''on'' or ''off''.' );                    
                end
                shadow = strcmpi( values{ii}, 'on' );
            otherwise
                error( 'UIExtras:Analyze4D:BadParameter', 'Unsupported optional parameter: %s.', params{ii} );
        end
    end
end
